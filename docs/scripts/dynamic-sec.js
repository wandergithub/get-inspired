...
function GenerateInstance(persona, persona2, container) {
  // Row container
  let div = CreateElement('div',['flex-row', 'contain'], container);
  // Instance 1
  let indiv = CreateElement('div',['flex-row', 'hidden'], div);
  let img = CreateElement('img',['inline'], indiv);
  img.setAttribute('src', persona.img);
  let info = CreateElement('div',['flex-column', 'info'], indiv);
  ...
  // Instance 2
  let indiv2 = CreateElement('div',['flex-row', 'hidden'], div);
  let img2 = CreateElement('img',['inline'], indiv2);
  img2.setAttribute('src', persona2.img);
  let info2 = CreateElement('div',['flex-column', 'info'], indiv2);
...
function DisplaySection() {
  let container = CreateElement('div', ['flex-column'], document.body);
  CreateElement('h2',['section-title', 'align-self-center'], container,'Developers');
  CreateElement('div',['guide-bar'],container);
  let speakersContainer = CreateElement('div',['flex-column'],container);
